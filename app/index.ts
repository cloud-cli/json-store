...
export default function start(port: number) {
  ...
  const onEsModule = (req, res) => res.set(esModule.headers).send(esModule.content.replace('__API_URL__', req.get('x-forwarded-host')));
  ...
  app.get('/', (req, res) => res.send(index.replace(/https:\/\/server-address.io/g, 'https://' + req.get('x-forwarded-host'))));
